...
function uninstallApp([string] $app) {
  ...
  try {
    ...
    printError "Failed to uninstall $app." "   "
  }
}

function uninstallWindowsDefaultApps {
  printInfo "Uninstalling default apps..."

  foreach ($app in $windowsDefaultApps) {
    uninstallApp $app
  }

  printSuccess "Default apps uninstalled."
}
